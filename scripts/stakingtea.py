...
LANG = {
    'vi': {
        'title': 'STAKE TEA - TEA TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        'enter_amount': 'Nhập số lượng dTEA để stake: ',
        'enter_count': 'Nhập số lần stake (mặc định 1): ',
        'processing_wallet': 'XỬ LÝ VÍ',
        'sending_tx': 'Đang gửi giao dịch...',
        'approving': 'Đang phê duyệt dTEA...',
    ...
    'en': {
        ...
        'found': 'Found',
        'wallets': 'wallets',
        'enter_amount': 'Enter dTEA amount to stake: ',
        'enter_count': 'Enter number of stakes (default 1): ',
        'processing_wallet': 'PROCESSING WALLET',
        'sending_tx': 'Sending transaction...',
        'approving': 'Approving dTEA...',
...
def display_balances(w3: Web3, address: str, staking_contract, language: str = 'en'):
    ...
    staked_balance = float(w3.from_wei(staking_contract.functions.stakedBalances(address).call(), 'ether'))
    ...
    print(f"{Fore.YELLOW}  {LANG[language]['staked_balance']:<12}: {staked_balance:.6f} dTEA{Style.RESET_ALL}")

async def approve_dtea(w3: Web3, private_key: str, amount: int, language: str = 'en'):
    account = Account.from_key(private_key)
    sender_address = account.address
    dtea_contract = w3.eth.contract(address=Web3.to_checksum_address(DTEA_ADDRESS), abi=DTEA_ABI)

    try:
        allowance = dtea_contract.functions.allowance(sender_address, CONTRACT_ADDRESS).call()
        if allowance >= amount:
            print(f"{Fore.GREEN}  ✔ Đã có đủ allowance: {w3.from_wei(allowance, 'ether')} dTEA{Style.RESET_ALL}")
            return True
        ...
        if balance_tea < gas_cost:
            print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance']}: Cần ít nhất {gas_cost:.6f} TEA cho gas{Style.RESET_ALL}")
            return False
        ...
        if receipt.status == 1:
            print(f"{Fore.GREEN}  ✔ {LANG[language]['approve_success']} | Tx: {tx_link}{Style.RESET_ALL}")
            return True
        else:
            print(f"{Fore.RED}  ✖ Phê duyệt thất bại | Tx: {tx_link}{Style.RESET_ALL}")
            return False
    except Exception as e:
        print(f"{Fore.RED}  ✖ Phê duyệt thất bại: {str(e)}{Style.RESET_ALL}")
        return False

async def stake_tea(w3: Web3, private_key: str, amount: float, tx_count: int, language: str = 'en'):
    account = Account.from_key(private_key)
    sender_address = account.address
    staking_contract = w3.eth.contract(address=Web3.to_checksum_address(CONTRACT_ADDRESS), abi=STAKING_ABI)

    amount_wei = w3.to_wei(amount, 'ether')
    total_amount_wei = amount_wei * tx_count

    dtea_contract = w3.eth.contract(address=Web3.to_checksum_address(DTEA_ADDRESS), abi=DTEA_ABI)
    balance_dtea = float(w3.from_wei(dtea_contract.functions.balanceOf(sender_address).call(), 'ether'))
    if balance_dtea < amount * tx_count:
        print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance']}: Cần ít nhất {amount * tx_count:.6f} dTEA{Style.RESET_ALL}")
        return 0

    if not await approve_dtea(w3, private_key, total_amount_wei, language):
        return 0
    ...
    for i in range(tx_count):
        try:
            ...
            if receipt.status == 1:
                ...
                print(f"{Fore.YELLOW}    {LANG[language]['amount']:<12}: {amount:.6f} dTEA{Style.RESET_ALL}")
                ...
                display_balances(w3, sender_address, staking_contract, language)
                successful_txs += 1
            else:
...
async def run(language: str = 'en'):
    ...
    for i, (profile_num, private_key) in enumerate(private_keys, 1):
        ...
        staking_contract = w3.eth.contract(address=Web3.to_checksum_address(CONTRACT_ADDRESS), abi=STAKING_ABI)
        display_balances(w3, account.address, staking_contract, language)
        print()
        ...
        successful_txs = await stake_tea(w3, private_key, amount, tx_count, language)
