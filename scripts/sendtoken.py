...
LANG = {
    'vi': {
        'title': 'SEND TOKEN ERC20 - TEA TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
    ...
    'en': {
        'title': 'SEND ERC20 TOKEN - TEA TESTNET',
        'info': 'Info',
        'found': 'Found',
        'wallets': 'wallets',
...
async def send_token(w3: Web3, private_key: str, wallet_index: int, contract_address: str, destination: str, amount: float, language: str = 'en'):
    ...
    try:
        ...
        print(f"{Fore.YELLOW}  - Số dư hiện tại: {balance:.6f} TEA{Style.RESET_ALL}")
        ...
        if balance < required_balance:
            print(f"{Fore.RED}  ✖ Insufficient balance for gas (Need: {required_balance:.6f} TEA){Style.RESET_ALL}")
            return False
