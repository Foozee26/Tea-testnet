...
LANG = {
    'vi': {
        'title': 'UNSTAKE TEA - TEA TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        'enter_amount': 'Nhập số lượng dTEA để unstake: ',
        'enter_count': 'Nhập số lần unstake (mặc định 1): ',
    ...
    'en': {
        ...
        'found': 'Found',
        'wallets': 'wallets',
        'enter_amount': 'Enter dTEA amount to unstake: ',
        'enter_count': 'Enter number of unstakes (default 1): ',
...
async def unstake_tea(w3: Web3, private_key: str, amount: float, tx_count: int, language: str = 'en'):
    ...
    staked_balance = float(w3.from_wei(staking_contract.functions.stakedBalances(sender_address).call(), 'ether'))
    if staked_balance < amount * tx_count:
        print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance']}: Cần ít nhất {amount * tx_count:.6f} dTEA đã stake{Style.RESET_ALL}")
        return 0
    ...
    for i in range(tx_count):
        try:
            ...
            gas_limit = staking_contract.functions.unstake(amount_wei).estimate_gas({'from': sender_address}) * 1.2
...
async def run(language: str = 'en'):
    ...
    for i, (profile_num, private_key) in enumerate(private_keys, 1):
        ...
        successful_txs = await unstake_tea(w3, private_key, amount, tx_count, language)
