...
LANG = {
    'vi': {
        'title': 'WITHDRAW TEA - TEA TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        'enter_amount': 'Nhập số lượng TEA để withdraw: ',
        'enter_count': 'Nhập số lần withdraw (mặc định 1): ',
        'processing_wallet': 'XỬ LÝ VÍ',
        'sending_tx': 'Đang gửi giao dịch...',
        'success': 'Withdraw thành công',
        'failure': 'Thất bại',
        'address': 'Địa chỉ',
        'amount': 'Số lượng',
        'gas': 'Gas',
    ...
    'en': {
        ...
        'found': 'Found',
        'wallets': 'wallets',
        'enter_amount': 'Enter TEA amount to withdraw: ',
        'enter_count': 'Enter number of withdrawals (default 1): ',
...
def display_balances(w3: Web3, address: str, withdraw_contract, language: str = 'en'):
    ...
    contract_balance = float(w3.from_wei(withdraw_contract.functions.balances(address).call(), 'ether'))
...
async def withdraw_tea(w3: Web3, private_key: str, amount: float, tx_count: int, language: str = 'en'):
    account = Account.from_key(private_key)
    sender_address = account.address
    withdraw_contract = w3.eth.contract(address=Web3.to_checksum_address(CONTRACT_ADDRESS), abi=WITHDRAW_ABI)

    amount_wei = w3.to_wei(amount, 'ether')
    total_amount_wei = amount_wei * tx_count

    contract_balance = float(w3.from_wei(withdraw_contract.functions.balances(sender_address).call(), 'ether'))
    if contract_balance < amount * tx_count:
        print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance']}: Cần ít nhất {amount * tx_count:.6f} TEA trong hợp đồng{Style.RESET_ALL}")
        return 0
    ...
    for i in range(tx_count):
        try:
            ...
            tx = withdraw_contract.functions.withdraw(amount_wei).build_transaction({
                'from': sender_address,
                'nonce': nonce,
                'chainId': CHAIN_ID,
                'gasPrice': gas_price
            })
            gas_limit = withdraw_contract.functions.withdraw(amount_wei).estimate_gas({'from': sender_address}) * 1.2
            ...
            if receipt.status == 1:
                ...
                display_balances(w3, sender_address, withdraw_contract, language)
                successful_txs += 1
            else:
...
async def run(language: str = 'en'):
    ...
    for i, (profile_num, private_key) in enumerate(private_keys, 1):
        ...
        withdraw_contract = w3.eth.contract(address=Web3.to_checksum_address(CONTRACT_ADDRESS), abi=WITHDRAW_ABI)
        display_balances(w3, account.address, withdraw_contract, language)
        print()
        ...
        successful_txs = await withdraw_tea(w3, private_key, amount, tx_count, language)
...
if __name__ == "__main__":
    asyncio.run(run_withdrawtea('vi'))
